...
pipeline {
    ...
    stages {
        ...
        stage('Set parameters') {
            steps {
                script {
                    env.TEMPLATE_PATH = "deployment/elb/elb_cfn.yaml"
                }
            }
        }
        ...
        stage('Deploy ELB') {
            steps {
                ...
                withAWS(role: "${TEST_ROLE}", roleAccount: "${TEST_ACCOUNT}", region: "${REGION}") {
                    ...
                    cfnUpdate(stack: "${STACK_NAME}", file: "${TEMPLATE_PATH}", params:["Env=${ENV}", "VpcStackName=${VPC_STACK}"], roleArn: "${TEST_ROLE}", tags: [ "Env=${ENV}" ], pollInterval: 6000, timeoutInMinutes: 20)
                }
            }
        }
    }
