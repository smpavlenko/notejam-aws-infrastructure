...
pipeline {
    ...
    stages {
        ...
        stage('Set parameters') {
            steps {
                script {
                    env.TEMPLATE_PATH = "additional/rds/rds_cfn.yaml"
                }
            }
        }
        ...
        stage('Deploy RDS') {
            steps {
                ...
                withAWS(role: "${TEST_ROLE}", roleAccount: "${TEST_ACCOUNT}", region: "${REGION}") {
                    ...
                    cfnUpdate(stack: "${STACK_NAME}", file: "${TEMPLATE_PATH}", params:["Env=${ENV}", "VpcStackName=${VPC_STACK}", "ElbStackName=${ELB_STACK}"], roleArn: "${TEST_ROLE}", tags: [ "Env=${ENV}" ], pollInterval: 6000, timeoutInMinutes: 20)
                }
            }
        }
    }
