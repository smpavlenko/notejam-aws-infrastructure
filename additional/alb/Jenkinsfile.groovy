...
pipeline {
    ...
    parameters {
        ...
        string(name: 'S3_BUCKET_NAME', defaultValue: '', description: '* S3 Bucket name for access logs', trim: true)
    ...
    stages {
        ...
        stage('Set parameters') {
            steps {
                script {
                    env.TEMPLATE_PATH = "additional/alb/alb_cfn.yaml"
                }
            }
        }
        ...
        stage('Deploy ALB') {
            steps {
                ...
                withAWS(role: "${TEST_ROLE}", roleAccount: "${TEST_ACCOUNT}", region: "${REGION}") {
                    ...
                    cfnUpdate(stack: "${STACK_NAME}", file: "${TEMPLATE_PATH}", params:["Env=${ENV}", "VpcStackName=${VPC_STACK}", "AccessLogsBucketName=${S3_BUCKET_NAME}"], roleArn: "${TEST_ROLE}", tags: [ "Env=${ENV}" ], pollInterval: 6000, timeoutInMinutes: 20)
                }
            }
        }
    }
